...
#     X0 = data.frame(matrix(0, nrow = 1, ncol = d))
#     if(d==1) names(X0) = "Xd" # weird work around because of stupid R simplifying when slicing a data.frame/matrix
#     tsk4 = makeRegrTask(data = df, target = "target")
#     trn = train(lrn.knn1.regr, tsk4)
#     y0.hat = getPredictionResponse(predict(trn, newdata = X0))
#     y0.hat.matrix[d, b] = y0.hat
#   }
# }
```

The bias-variance decomposition panel is the most complicated. We have to calculate the expected value of the target value for **x** = (0, …, 0) predicted by 1-nearest-neighbor given a sample of *n* = 1000 observations. We estimate the expected value by drawing 10000 samples with 1000 observations, calculating the target value of the nearest neighbor to zero and the empirical mean of these predictions.
