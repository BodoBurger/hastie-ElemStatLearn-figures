...
```{r figure-02-04-test-error, fig.asp=1}
...
```{r figure-02-06-curse, fig.asp=1}
...
Figure 2-11 in the book seems to be more like a theoretical sketch than the result of a
simulation which is what we do here to illustrate the bias-variance tradeoff.

```{r figure-02-11-dgp, message=FALSE, warning=FALSE}
n = 100
...
We try to fit the data using polynomial regression. We increase the model complexity
step by step (i.e. increasing the degree of the polynomial) and look at how train and test
error evolve.

```{r figure-02-11-data, message=FALSE, warning=FALSE}
...
```{r figure-02-11-compare-poly, warning=FALSE}
...
The simple linear model underfits, the polynomial of degree 20 overemphasizes the noise.
The polynomial of degree 7 seems to be a reasonable choice here.

# Bonus figure: naive Bayes classifier

```{r figure-bonus-02-naive-bayes}
...
```{r figure-bonus-02-knn67, fig.asp=1}
