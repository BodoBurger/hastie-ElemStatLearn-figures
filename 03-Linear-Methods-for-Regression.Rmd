...
bestsub.mse = numeric(p)

for (i in 1:p) {
  b.hat = numeric(p)
  coef.tmp = coef(bestsub.model, i)
  #b.hat[as.integer(substring(names(coef.tmp), i))] = coef.tmp
  b.hat[summary(bestsub.model)$which[i,]] = coef.tmp
  bestsub.mse[i] = sum((b.hat - dgp$b)^2)
}

coef(bestsub.model, 1)
coef(forstep.model, 31)
dgp$b
bestsub.model.summary = summary(bestsub.model)
